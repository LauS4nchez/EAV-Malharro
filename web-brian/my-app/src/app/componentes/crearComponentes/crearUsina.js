...
// Formulario para crear una nueva usina con nombre, carrera, link e imagen
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/usinas`, {
        ...
        body: JSON.stringify({
          data: { nombre, carrera, link, imagen: imagenId },
        }),
      });
    ...
    <div className="form-container">
      <form onSubmit={handleSubmit} className="form-in-container">
        ...
        <div className="form-group">
          ...
          <input className="form-input" type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Nombre" required />
        </div>

        <div className="form-group">
          <label className="block form-label">Carrera</label>
          <input className="form-input" type="text" value={carrera} onChange={(e) => setCarrera(e.target.value)} placeholder="Carrera" required />
        </div>
        
        <div className="form-group">
          <label className="block form-label">Contacto</label>
          <input className="form-input" type="url" value={link} onChange={(e) => setLink(e.target.value)} placeholder="Link" required />
        </div>
        
        <div className="form-group">
          <label className="block form-label">Trabajo</label>
          <input className="form-input" type="file" onChange={(e) => setImagen(e.target.files[0])} accept="image/*" required />
        </div>
